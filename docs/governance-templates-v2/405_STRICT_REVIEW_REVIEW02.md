...
- **Start from `git status --short`.** A file the result claims to have
  changed that is absent there was not changed — that alone is REJECTED.
...
1. Write question to: `{bridge_dir}/strict_review/escalations/{ID}-review02-question.md`
2. Signal escalation:
   ```bash
   python3 {project_root}/scripts/bridgeV002/dispatch.py \
