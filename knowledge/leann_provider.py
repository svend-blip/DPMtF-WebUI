...
``sys.modules`` and DPMtF keeps importing and running when LEANN is absent.
"""
...
class LeannProvider(KnowledgeProvider):
    ...
    manifest written by ``knowledge.indexer`` (one JSON object per line with
    at least ``path`` and ``content`` keys, plus an optional ``scope``). The
    manifest records are handed to LEANN's builder with stable passage ids
    derived from ``scope`` and ``path``, and the manifest ``path``/``scope``
    values are stored in each passage's metadata so search results can carry
    usable source references.
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                print(
                    "knowledge.leann_provider: warning: searcher cleanup "
                    f"failed: {exc}",
                    file=sys.stderr,
                )
    ...
    def _read_manifest(source: str) -> list[dict[str, Any]]:
        """Read and validate a JSONL manifest written by ``knowledge.indexer``."""
